...
Перем ИзданиеПлатформы Экспорт;
...
Функция ПолучитьИдентификаторПроцессаОС() Экспорт
	...
	Если Не ЗначениеЗаполнено(ИдентификаторПроцессаОС) Тогда
		...
		//ДочернийПроцесс = oShell.Exec("rundll32.exe Shell32,ShellAboutA");
...
Функция СоздатьОбъектВнешнихМетаданных(ПолноеИмяФайла, МенеджерВнешнихОбъектов = Неопределено, Конвертировать = Истина) Экспорт
	...
	Исключение
		...
		Если Истина
			...
			//И ИзданиеПлатформы <> 802015 // Антибаг 8.2.15 http://partners.v8.1c.ru/forum/thread.jsp?id=1003164#1003164
		Тогда
...
Функция ПолучитьИДВерсииПлатформы(СистемнаяИнформация = Неопределено) Экспорт
	...
	//ИзданиеПлатформы = Число(МассивФрагментов[0]) * 1000 * 1000 + Число(МассивФрагментов[1]) * 1000 + Число(МассивФрагментов[2]);
...
Функция ПолучитьТаблицуСвойствОбъектаИнформатором(Объект, ТипСлов = "Свойство", НеопределеноСчитатьГлобальнымКонтекстом = Истина) Экспорт
	...
	Перем ТЗ;
	Если Ложь
		//Или ИзданиеПлатформы > 803001 // На 8.3.2 информатор падает
		//Или ИзданиеПлатформы >= 802018 // На 8.2.18 информатор падает
...
ИзданиеПлатформы = Число(МассивФрагментов[0]) * 100 * 1000 + Число(МассивФрагментов[1]) * 1000 + Число(МассивФрагментов[2]);
