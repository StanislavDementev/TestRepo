...
Функция ПолучитьСписокРеквизитовОбъектаБДЛкс(пОбъект, ЛиВключатьТолькоЧитаемые = Ложь,
	...
	Для Каждого МетаРеквизит Из ОбъектМетаданных.Реквизиты Цикл
		...
		КонецЕсли;
	КонецЦикла;
	Если ирКэш.Получить().ИзданиеПлатформы >= 802014 Тогда
	...
	Если ТаблицаИндикаторов.Количество() = 0 Тогда
		#Если Клиент Тогда
			Если ирКэш.ЛиМобильныйРежимЛкс() Тогда
				ирМобильный.ПолучитьФорму().ПодключитьОбработчикОжидания("ОсвободитьВсеИндикаторыПроцессовЛкс", 0.1, Истина);
			Иначе
...
Процедура ОтладитьОтложенныйОбъектЛкс(СсылкаИлиАдрес = Неопределено, УдалитьОбъектПослеУспешногоОткрытия = Ложь) Экспорт
	
	#Если _ Тогда
	    Ссылка = Справочники.ирОбъектыДляОтладки.ПустаяСсылка();
	#КонецЕсли
	Если СсылкаИлиАдрес = Неопределено Тогда
		СсылкаИлиАдрес = ПолучитьТекстИзБуфераОбменаОСЛкс();
		Если Не ВвестиСтроку(СсылкаИлиАдрес, "Введите результат сохранения объекта") Тогда 
			Если Не ирКэш.ЛиМобильныйРежимЛкс() Тогда
				СсылкаИлиАдрес = ВыбратьСсылкуЛкс(Метаданные.Справочники.ирОбъектыДляОтладки);
			КонецЕсли; 
		КонецЕсли;
		Если Не ЗначениеЗаполнено(СсылкаИлиАдрес) Тогда
			Возврат;
		КонецЕсли;
		Если ТипЗнч(СсылкаИлиАдрес) = Тип("Строка") Тогда
			ВычислительРегулярныхВыражений = ирКэш.Получить().RegExp;
			ВычислительРегулярныхВыражений.Pattern = "e1cib/tempstorage/\b[A-F0-9]{8}(?:-[A-F0-9]{4}){3}-[A-F0-9]{12}\b";
			Результат = ВычислительРегулярныхВыражений.Execute(СсылкаИлиАдрес);
			...
			Если Результат.Count = 0 Тогда 
				...
				Возврат;
			КонецЕсли; 
			СсылкаИлиАдрес = Результат.Item(0).Value;
			Если ЭтоАдресВременногоХранилища(СсылкаИлиАдрес) Тогда
				Если Не ирКэш.ЛиМобильныйРежимЛкс() Тогда
					ОтложитьУпакованныйОбъектДляОтладкиЛкс(ПолучитьИзВременногоХранилища(СсылкаИлиАдрес), СсылкаИлиАдрес);
				КонецЕсли; 
			Иначе
				СсылкаИлиАдрес = Справочники.ирОбъектыДляОтладки.ПолучитьСсылку(Новый УникальныйИдентификатор(СсылкаИлиАдрес));
				Если Не ЗначениеЗаполнено(СсылкаИлиАдрес) Тогда
					Возврат;
				КонецЕсли; 
			КонецЕсли; 
	...
	КонецЕсли; 
	Если ТипЗнч(СсылкаИлиАдрес) = Тип("Строка") Тогда
		СтрокаРезультата = ПолучитьИзВременногоХранилища(СсылкаИлиАдрес);
	...
	Иначе
		...
		|";
		Запрос.УстановитьПараметр("Ссылка", СсылкаИлиАдрес);
	...
	Если УдалитьОбъектПослеУспешногоОткрытия Тогда
		Если ТипЗнч(СсылкаИлиАдрес) = Тип("Строка") Тогда
			УдалитьИзВременногоХранилища(СсылкаИлиАдрес);
		Иначе
			УдалениеОбъекта = Новый УдалениеОбъекта(СсылкаИлиАдрес);
...
Функция ПроверитьЗапуститьОтладчик(Знач ВремяОжиданияЗапуска = 5) Экспорт
	...
	Если ИдентификаторПроцессаОтладчика = Неопределено Тогда
		...
		Если Платформа.ИзданиеПлатформы = 802015 Тогда 
...
Функция ОткрытьТекущуюСтрокуТабличногоПоляТаблицыБДВРедактореОбъектаБДЛкс(ТабличноеПоле, ПолноеИмяМД = Неопределено, ДоступныеПоляВыбора = Неопределено) Экспорт
	...
	ФормаРедактора.	мПараметрКлючОбъекта = КлючОбъекта;
	ФормаРедактора.Открыть();
...
Функция ОтложитьУпакованныйОбъектДляОтладкиЛкс(СтруктураПараметров, выхОбъектДляОтладки = Неопределено)
	...
	Тогда
        ...
        + " Создан объект """ + ОбъектДляОтладки + """(" + выхОбъектДляОтладки.УникальныйИдентификатор() + ")";
    Иначе
